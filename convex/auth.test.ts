...
test("Convex Auth Backend Security and Role Enforcement Tests", async () => {
  ...
  const unauthedProfile = await t.query(api.auth.users.getCurrentUserProfile);
  expect(unauthedProfile).toBeNull();

  await expect(
    t.mutation(api.auth.users.updateProfile, { username: "hacker" })
  ...
  const studentId = await studentAuth.mutation(api.auth.users.storeUser, {
    username: "arch_jane",
    firstName: "Jane",
    lastName: "Doe",
  });
  ...
  const studentProfile = await studentAuth.query(api.auth.users.getCurrentUserProfile);
  ...
  await expect(
    studentAuth.mutation(api.auth.users.updateRole, {
  ...
  const updateResult = await adminAuth.mutation(api.auth.users.updateRole, {
    targetUserId: studentId,
    newRole: "content_manager",
  });
  expect(updateResult.success).toBe(true);

  const updatedRole = await studentAuth.query(api.auth.users.getRole);
  ...
  await expect(
    studentAuth.mutation(api.auth.users.updateProfile, { username: "new_name" })
