...
test("Notification & Study Room Invite System Tests", async () => {
  ...
  const userAId = await userA.mutation(api.users.storeUser, {
    username: "alice",
  });

  const userB = t.withIdentity({ subject: "user_b_sub" });
  const userBId = await userB.mutation(api.users.storeUser, {
    username: "bob",
  });

  // User A creates a study room
  const roomRes = await userA.mutation(api.rooms.createStudyRoom, {
  ...
  const inviteRes = await userA.mutation(api.notifications.inviteUserToStudyRoom, {
  ...
  const userBNotifications = await userB.query(api.notifications.getUserNotifications, {
    unreadOnly: true,
  });
  ...
  const markRes = await userB.mutation(api.notifications.markAsRead, {
    notificationId: userBNotifications[0]._id,
  });

  expect(markRes.success).toBe(true);

  const remainingUnread = await userB.query(api.notifications.getUserNotifications, {
    unreadOnly: true,
  });
